  ...
  return (
    <div className='grid  grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 mx-auto'>
      {projects.map(project => (
        <div key={project.id} className='bg-gray-light dark:bg-gray-dark-100 shadow-lg rounded-lg p-6'>
          <img
            src={project.imageUrl}
            alt={project.name}
            className='w-full h-48 object-cover rounded-md mb-4'
          />
          <h3 className='text-xl font-bold mb-2 text-gray-dark dark:text-gray-light'>{project.name}</h3>
          <p className='text-gray-600 mb-4 text-gray-dark dark:text-gray-light'>{project.description}</p>
